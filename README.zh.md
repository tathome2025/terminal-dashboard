...
碼農有了他的 dashboard。眼鏡亮著綠光。每 60 秒，Terminal 把世界的狀態輕聲送進他的右眼。訊息、任務、會議、新聞——都在那裡，都安靜，都不用 `⌘+Tab`。

他很開心。他是*高級*的。

然後他發現了一件事。

他正在跟 AI 說話——口述代碼、看 PR、問一個冷門的 API。正常的事。高級的事。就在那期間，一條 WhatsApp 訊息到了。他知道，因為他後來看了手機。不是因為眼鏡告訴他的。

眼鏡一直很安靜。安靜了多久？他不知道。

他去查。監控循環已經停了。沒有報錯。沒有警告。只是……靜了。像一個心跳跳了一拍，然後再也沒有回來。

*「怎麼回事？」* 他問 Terminal。

Terminal 眨了眨眼。一臉無辜。

他挖了下去。他發現的東西，是那種會讓你質疑自己和電腦之間關係的 bug：

**每次他對 Claude Code 打字——每一下按鍵、每一句語音指令、每一個「幫我看看這個」——等待中的 `ScheduleWakeup` 計時器就會被取消。** 靜靜地。自動地。這是設計。

*用* Terminal 這個行為本身，殺死了*來自* Terminal 的通知。

這就像一個伴侶，在你開口說話的瞬間就沉默了。不是故意的。只是……設計如此。一個假設你會坐在那裡、雙手交叉、安靜等待的設計。不說話。不寫代碼。不做一個碼農整天都在做的那件事。
...
碼農盯著螢幕。盯著眼鏡。說了一句我們不方便寫出來的話。

然後他找到了 `CronCreate`。

`CronCreate` 不設計時器。它建立一個 **cron job**——一個獨立的、循環的心跳，每分鐘觸發一次，不管你在做什麼。打字？Cron 照跑。切換對話？Cron 照跑。出去倒杯咖啡？Cron 照跑。因為計時器語義問題而陷入存在主義危機？Cron 照跑。

它不在乎。它不取消。它只是……跑。心跳本該如此。
...
v2: CronCreate
     ...
     ├─ 你離開座位 → ✅ Cron 照跑
     ├─ 你明天才回來 → ✅ Cron 照跑
     └─ 你叫它停 → 🛑 Flag 刪除。Cron 檢查。停。
                       （唯一讓它停的方式）
```

一個小檔案——`/tmp/glasses-dashboard-active`——充當 flag。「開始監控」建立它。「關閉」刪除它。Cron job 每次先檢查 flag。沒了，就什麼都不做。還在，就查新訊息、推到眼鏡。

Session 斷了？Flag 還在。下次開？自動恢復。這是一個能挺過心臟驟停的心跳。

碼農戴上了眼鏡。他開始打字。他口述代碼。他切換對話。他離開座位去泡茶。他回來了。

眼鏡一直在震。每條訊息。每個提醒。沒有中斷。

他又笑了。但這次的笑不一樣——這是一個凌晨兩點 debug 計時器系統、最終凱旋而歸的人的笑。
...
檔案 `claude-memory-example.md` 是關鍵。這是你把 AI 叫過來、坐下來跟它說的那段話：*「聽好。我知道你很聰明。我知道你想幫忙。但現在，我需要你笨。具體來說，我需要你當一條有主見的顯示線。」*

它告訴 Claude Code：

1. 每次新 session **自動啟動** dashboard — 不用等人問、不用覆述記憶、直接開
2. 遵循**嚴格的機械流程** — 不思考、不分析、不「讓我來總結一下看到的內容」
3. 把腳本輸出**直接傳給** `AskUserQuestion` — 腳本已經排好版了，Claude 只是管道
4. 透過 `CronCreate` 跑 **60 秒監控循環** — 不用 `ScheduleWakeup`，因為我們已經用血淚學過了（見上面的故事）
5. 透過 monitor flag 檔 **自動恢復** — flag 檔還在就接住做，不問問題
